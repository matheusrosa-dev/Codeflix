...
export class GenreSequelizeRepository implements IGenreRepository {
	...
	async search(props: GenreSearchParams): Promise<GenreSearchResult> {
		...
		if (
			props.searchTerm &&
			(props.searchTerm.name || props.searchTerm.categories_id)
		) {
			if (props.searchTerm.name) {
				wheres.push({
					field: "name",
					value: `%${props.searchTerm.name}%`,
					get condition() {
						return {
							[this.field]: {
								[Op.like]: this.value,
		...
		const count = await this.genreModel.count({
			...
			include: [props.searchTerm?.categories_id && "categories_id"].filter(
				(i) => i,
			),
