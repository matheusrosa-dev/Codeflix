...
describe("UpdateGenreUseCase Unit Tests", () => {
  ...
  describe("execute method", () => {
    it("should throw an entity validation error when categories id not exists", async () => {
      expect.assertions(3);
      const genre = Genre.fake().oneGenre().build();
      ...
      try {
        ...
        expect(e.error).toStrictEqual([
          {
            categories_id: [
              "Category Not Found using IDs: 4f7e1c30-3f7a-4f51-9f4a-3e9c4c8f1a1a",
              "Category Not Found using IDs: 7f7e1c30-3f7a-4f51-9f4a-3e9c4c8f1a1a",
            ],
          },
        ]);
      }
    ...
    it("should update a genre", async () => {
      const category1 = Category.fake().oneCategory().build();
      const category2 = Category.fake().oneCategory().build();
