import { CastMembersIdExistsInDatabaseValidator } from "../../../../cast-member/app/validations/cast-members-ids-exists-in-database.validator";
import { CategoriesIdExistsInDatabaseValidator } from "../../../../category/app/validations/categories-ids-exists-in-database.validator";
import { GenresIdExistsInDatabaseValidator } from "../../../../genre/app/validations/genres-ids-exists-in-database.validator";
  ...
  implements IUseCase<CreateVideoInput, CreateVideoOutput>
{
  ...
  async execute(input: CreateVideoInput): Promise<CreateVideoOutput> {
    ...
    const [eitherCategoriesId, eitherGenresId, eitherCastMembers] =
      await Promise.all([
        this.categoriesIdValidator.validate(input.categories_id),
        this.genresIdValidator.validate(input.genres_id),
        this.castMembersIdValidator.validate(input.cast_members_id),
      ]);
