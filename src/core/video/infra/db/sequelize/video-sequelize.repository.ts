...
export class VideoSequelizeRepository implements IVideoRepository {
	...
	async search(props: VideoSearchParams): Promise<VideoSearchResult> {
		...
		const videoGenreRelation = this.videoModel.associations.genres_id.target;
		const videoGenreTableName = videoGenreRelation.getTableName() as string;
		const videoCastMemberRelation =
			this.videoModel.associations.cast_members_id.target;
		const videoCastMemberTableName =
			videoCastMemberRelation.getTableName() as string;
		...
		if (
			props.searchTerm &&
			(props.searchTerm.title ||
				props.searchTerm.categories_id ||
				props.searchTerm.genres_id ||
				props.searchTerm.cast_members_id)
		) {
			if (props.searchTerm.title) {
				wheres.push({
					field: "title",
					value: `%${props.searchTerm.title}%`,
					get condition() {
						return {
							[this.field]: {
								[Op.like]: this.value,
			...
			if (props.searchTerm.categories_id) {
				wheres.push({
					field: "categories_id",
					value: props.searchTerm.categories_id.map((c) => c.id),
					get condition() {
						return {
			...
			if (props.searchTerm.genres_id) {
				wheres.push({
					field: "genres_id",
					value: props.searchTerm.genres_id.map((c) => c.id),
					get condition() {
						return {
							["$genres_id.genre_id$"]: {
								[Op.in]: this.value,
			...
			if (props.searchTerm.cast_members_id) {
				wheres.push({
					field: "cast_members_id",
					value: props.searchTerm.cast_members_id.map((c) => c.id),
					get condition() {
						return {
		...
		const count = await this.videoModel.count({
			distinct: true,
			include: [
				props.searchTerm?.categories_id && "categories_id",
				props.searchTerm?.genres_id && "genres_id",
				props.searchTerm?.cast_members_id && "cast_members_id",
