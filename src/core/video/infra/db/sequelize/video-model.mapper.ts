...
export class VideoModelMapper {
	static toEntity(model: VideoModel) {
		...
		const bannerModel = image_medias.find(
			(i) => i.video_related_field === ImageMediaRelatedField.BANNER,
		);
		...
		const thumbnailModel = image_medias.find(
			(i) => i.video_related_field === ImageMediaRelatedField.THUMBNAIL,
		);
		...
		const thumbnailHalfModel = image_medias.find(
			(i) => i.video_related_field === ImageMediaRelatedField.THUMBNAIL_HALF,
		);
		...
		const trailerModel = audio_video_medias.find(
			(i) => i.video_related_field === AudioVideoMediaRelatedField.TRAILER,
		);
		...
		const videoModel = audio_video_medias.find(
			(i) => i.video_related_field === AudioVideoMediaRelatedField.VIDEO,
		);
