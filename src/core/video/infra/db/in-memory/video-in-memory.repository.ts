	...
	implements IVideoRepository
{
	...
	protected async applySearchTerm(
		...
		return items.filter((i) => {
			...
			const containsGenresId = searchTerm.genres_id?.some((c) =>
				i.genres_id.has(c.id),
			);
			const containsCastMembersId = searchTerm.cast_members_id?.some((c) =>
				i.cast_members_id.has(c.id),
			);
