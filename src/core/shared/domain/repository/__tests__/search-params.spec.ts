...
describe("SearchParams Unit Tests", () => {
	...
	test.each([
		{ searchTerm: null, expected: null },
		{ searchTerm: undefined, expected: null },
		{ searchTerm: "", expected: null },

		{ searchTerm: 0, expected: "0" },
		{ searchTerm: -1, expected: "-1" },
		{ searchTerm: 5.5, expected: "5.5" },
		{ searchTerm: true, expected: "true" },
		{ searchTerm: false, expected: "false" },
		{ searchTerm: {}, expected: "[object Object]" },
		{ searchTerm: "field", expected: "field" },
	])("searchTerm props %p", (props) => {
		const params = new SearchParams();
		expect(params.searchTerm).toBeNull();

		expect(
			new SearchParams({ searchTerm: props.searchTerm as any }).searchTerm,
		).toBe(props.expected);
	});
});
