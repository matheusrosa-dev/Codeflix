	...
	describe("search method tests", () => {
		it("should order by created_at DESC when search params are null", async () => {
			const castMembers = CastMember.fake()
				.manyCastMembers(16)
				.withCreatedAt((index) => new Date(Date.now() + 100 + index))
				.build();
		...
		it("should apply paginate and searchTerm by name", async () => {
			const castMembers = [
				CastMember.fake()
					.oneActor()
					.withName("test")
					.withCreatedAt(new Date(Date.now() + 4000))
					.build(),
				CastMember.fake()
					.oneActor()
					.withName("a")
					.withCreatedAt(new Date(Date.now() + 3000))
					.build(),
				CastMember.fake()
					.oneActor()
					.withName("TEST")
					.withCreatedAt(new Date(Date.now() + 2000))
					.build(),
				CastMember.fake()
					.oneActor()
					.withName("TeSt")
					.withCreatedAt(new Date(Date.now() + 1000))
					.build(),
			];
			await repository.bulkInsert(castMembers);
		...
		it("should apply paginate and searchTerm by type", async () => {
		...
		describe("should search using searchTerm by name, sort and paginate", () => {
			const castMembers = [
		...
		describe("should search using searchTerm by type, sort and paginate", () => {
			const castMembers = [
				CastMember.fake().oneActor().withName("test").build(),
		...
		describe("should search using searchTerm by name and type, sort and paginate", () => {
			const castMembers = [
				CastMember.fake().oneActor().withName("test").build(),
