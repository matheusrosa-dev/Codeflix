...
export class CastMemberSequelizeRepository implements ICastMemberRepository {
  ...
  async search(props: CastMemberSearchParams): Promise<CastMemberSearchResult> {
    ...
    if (props.searchTerm && (props.searchTerm.name || props.searchTerm.type)) {
      if (props.searchTerm.name) {
        where["name"] = { [Op.like]: `%${props.searchTerm.name}%` };
      }

      if (props.searchTerm.type) {
        where["type"] = props.searchTerm.type.type;
      }
    }
