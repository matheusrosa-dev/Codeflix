...
describe("CastMemberFakerBuilder Unit Tests", () => {
	describe("cast_member_id prop", () => {
		...
		test("should pass index to cast_member_id factory", () => {
			...
			const fakerMany = CastMemberFakeBuilder.manyCastMembers(2);
	...
	describe("type prop", () => {
		...
		test("withType", () => {
			const director = CastMemberType.createOneDirector();
			const $this = faker.withType(director);
			expect($this).toBeInstanceOf(CastMemberFakeBuilder);
			expect(faker.type).toEqual(director);

			const actor = CastMemberType.createOneActor();
	...
	describe("created_at prop", () => {
		...
		test("should throw error when any with methods has called", () => {
			const fakerCastMember = CastMemberFakeBuilder.oneActor();
	...
	test("should create a cast member", () => {
		...
		castMember = faker
			...
			.withName("name test")
			.withType(CastMemberType.createOneDirector())
