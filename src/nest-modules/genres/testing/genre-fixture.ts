...
export class CreateGenreFixture {
	...
	static arrangeForSave() {
		const faker = Genre.fake().oneGenre().withName("test name");
			...
			NAME_TOO_LONG: {
				...
				expected: {
					message: [
						...
						"Category Not Found using IDs: d8952775-5f69-42d5-9e94-00f097e1b98c",
					],
					...defaultExpected,
				},
			},
...
export class ListGenresFixture {
	static arrangeIncrementedWithCreatedAt() {
		...
		const _entities = Genre.fake()
			...
			.withName((i) => `${i}`)
			.withCreatedAt((i) => new Date(Date.now() + i * 2000))
			.build();
	...
	static arrangeUnsorted() {
		...
		const arrange_searchTerm_by_name_sort_name_asc = [
			{
				send_data: {
					page: 1,
					per_page: 2,
		...
		const arrange_searchTerm_by_categories_id_and_sort_by_created_desc = [
			{
				send_data: {
					page: 1,
					per_page: 2,
				...
				get label() {
					return JSON.stringify({
						...this.send_data,
						searchTerm: { categories_id_length: 1 },
					});
				},
				expected: {
			...
			},
			{
				...
				get label() {
					return JSON.stringify({
						...this.send_data,
						searchTerm: { categories_id_length: 2 },
					});
				},
				expected: {
					entities: [entitiesMap.TeSt, entitiesMap.TEST],
		...
		return {
			arrange: [
				...arrange_searchTerm_by_name_sort_name_asc,
				...arrange_searchTerm_by_categories_id_and_sort_by_created_desc,
			],
			entitiesMap,
			relations,
		};
