...
import { CastMemberOutputMapper } from "@core/cast-member/app/use-cases/common/cast-member-output";
...
describe("CastMembersController Integration Tests", () => {
	...
	describe("should create a cast member", () => {
		...
		test.each(arrange)("when body is $send_data", async ({
			send_data,
			expected,
		}) => {
	...
	describe("should update a cast member", () => {
		...
		test.each(arrange)("with request $send_data", async ({
			send_data,
			expected,
		}) => {
	...
	describe("search method", () => {
		...
		describe("should returns output using pagination, sort and filter", () => {
			...
			test.each(
				arrange,
			)('when send_data is {"filter": $send_data.filter, "page": $send_data.page, "per_page": $send_data.per_page, "sort": $send_data.sort, "sort_dir": $send_data.sort_dir}', async ({
				send_data,
				expected,
			}) => {
				const presenter = await controller.search(send_data);
